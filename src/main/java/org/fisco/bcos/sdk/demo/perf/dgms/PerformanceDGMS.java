...
public class PerformanceDGMS {
	...
	public static void start(String groupId, int count, Integer qps)
		...
		String address;
		try {
			List<Type> result = codec.decodeMethodAndGetOutputObject(Factory.getABI(), "deployOne",
					receipt.getOutput());
		...
		OneToOne oneToOne = OneToOne.load(address, client, client.getCryptoSuite().getCryptoKeyPair());
		...
		IntStream.range(0, count).parallel().forEach(i -> {
			...
			oneToOne.issueAsset(users[i].getName(), BigInteger.valueOf(users[i].getValue()), "This is a test asset",
					"This is a test asset data".getBytes(), new TransactionCallback() {
						@Override
						public void onResponse(TransactionReceipt receipt) {
							if (!receipt.isStatusOK()) {
								System.out.println(
										"Send transaction error! " + receipt.getStatus() + " " + receipt.getMessage());
							}
		...
		IntStream.range(0, count).parallel().forEach(i -> {
			limiter.acquire();
			try {
				String user = oneToOne.ownerOf(BigInteger.valueOf(users[i].getValue()));
